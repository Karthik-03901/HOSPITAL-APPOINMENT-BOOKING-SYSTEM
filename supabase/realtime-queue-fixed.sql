-- =====================================================
-- REAL-TIME QUEUE TRACKING SYSTEM (FIXED)
-- =====================================================
...
-- =====================================================
-- Step 2: Enable Realtime (Safe Method)
-- =====================================================

-- Check and enable realtime safely
DO $$ 
BEGIN
  -- Add queue_positions to realtime publication if not already added
  IF NOT EXISTS (
    ...
    EXECUTE 'ALTER PUBLICATION supabase_realtime ADD TABLE queue_positions';
    RAISE NOTICE '✅ Added queue_positions to realtime';
  ELSE
    RAISE NOTICE '⚠️  queue_positions already in realtime';
  END IF;
  ...
  ELSE
    RAISE NOTICE '⚠️  appointments already in realtime';
  END IF;
END $$;
...
CREATE POLICY "queue_select_own" ON queue_positions
  ...
  USING (
    appointment_id IN (
      ...
      WHERE patient_email = current_setting('request.jwt.claims', true)::json->>'email'
    )
  );
...
BEGIN
  ...
  v_department := COALESCE(NEW.department_name, 'General');
  ...
  FROM appointments
  WHERE COALESCE(department_name, 'General') = v_department
  ...
  IF (TG_OP = 'UPDATE' AND NEW.status IN ('completed', 'cancelled', 'no_show', 'consulting')) THEN
    ...
    WITH ranked AS (
      SELECT 
        qp.id as queue_id,
        ROW_NUMBER() OVER (
          PARTITION BY COALESCE(a.department_name, 'General'), a.appointment_date::DATE 
          ORDER BY a.appointment_time
        ) as new_position
      FROM appointments a
      JOIN queue_positions qp ON qp.appointment_id = a.id
      WHERE COALESCE(a.department_name, 'General') = COALESCE(NEW.department_name, 'General')
...
BEGIN
  ...
  FROM appointments a
  JOIN queue_positions qp ON qp.appointment_id = a.id
  WHERE COALESCE(a.department_name, 'General') = p_department
...
BEGIN
  ...
  RAISE NOTICE '1. Open test-realtime-queue.html';
  RAISE NOTICE '2. Click "Create Test Appointment"';
  RAISE NOTICE '3. Watch real-time updates!';
  RAISE NOTICE '============================================';
  RAISE NOTICE '';
END $$;
